...
LEFT OUTER JOIN ods_central.dw_country ON co.country=DW_COUNTRY.COUNTRYCODE and dw_country.ods_current_flag=1
  where   requestdate between '01-JAN-2019' and '30-OCT-2019'
...
-- 12:04 minutes
--124M rows
create table RA_nextcgroups as        
...
-- 2:05:09 hours
--124M rows
create table select * from RA_cgroupcounts as        
(select distinct ra_nextcgroups.*,         
count(collectionkey) OVER (partition by RA_nextcgroups.customerkey, collectiongroup) AttemptsInGroup,        
max(collectionkey) OVER (partition by RA_nextcgroups.customerkey, collectiongroup) FinalAttempt 
from RA_nextcgroups         
JOIN ods_central.j2_ISPCOLLECTION co ON RA_nextcgroups.customerkey = co.customerkey and collectionkey >= collectiongroupkey         
...
--124M rows
create table RA_cgroupsdisp as        
(select distinct  RA_cgroupcounts.*,    
...
when DISP.RESPONSE='D' and retrycount < 4 and collectiongroup = max(collectiongroup) OVER (partition by RA_cgroupcounts.customerkey) then 'Declining'        
...
when DISP.RESPONSE='D' and retrycount < 4 and collectiongroup = max(collectiongroup) OVER (partition by RA_cgroupcounts.customerkey) then 'Declining through try '||to_char(retrycount+1) || ' (Attempt in group  TBD)'      
...
from  RA_cgroupcounts         
JOIN ods_central.j2_ISPCOLLECTION disp ON RA_cgroupcounts.customerkey = disp.customerkey and collectionkey=finalattempt-->= collectiongroupkey and collectionkey < nextgroupkey        
where ods_current_flag  = 1    
 ...
 create table RA_Final as 
  ...
  -- collectiondate,it's in here but doesn't DO anything.
   ...
   declinesuntilsuccessful,
       ...
       --collectionkey, 
       RA_cgroupsdisp.collectiongroupkey , RA_cgroupsdisp.nextgroupkey,
     --  RA_cgroupsdisp.* --, custsubset.*
RA_CGROUPSDISP.COLLECTIONGROUPSTARTDATE,  
RA_CGROUPSDISP.COLLECTIONGROUP,
RA_CGROUPSDISP.AttemptsInGroup,
RA_CGROUPSDISP.FinalAttempt FinalCollectionAttemptKey,
RA_CGROUPSDISP.Group_Disposition,
RA_CGROUPSDISP.group_disposition_detail
...
--select * from ra_final where customerkey = 50134715
select * from RA_cgroupsdisp where customerkey = 50134715

select trunc(requestdate,'MONTH'), count(*) from RA_Final
