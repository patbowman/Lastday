CREATE OR REPLACE PROCEDURE ODS_CENTRAL.ods_retryanalysis(
...
--   v_fraud_filter   VARCHAR (67);
   ...
   v_groupdate      date; -- date to go back for checking groups
   v_ods_date       DATE;
   v_disp_length    NUMBER :=90; -- number of days to go back to check for disposition
   v_test_flag      NUMBER := 0; -- set to 1 for testing.
...
if v_test_flag = 1 then
        V_startdate := '11-JAN-2016';
        V_enddate := v_startdate +10;
end if;

-- used by GT_RAD_dispattempts_ex2
v_groupdate := v_enddate-v_disp_length;
...
insert into  ods_central.GT_rad_newattempts_ex1 
...
--commit;
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_dispattempts_ex2';
if v_test_flag = 1 then
...
and requestdate between v_groupdate and V_enddate -- right now, use a year before start date, though that's overkill.
and ods_current_flag =1;
...
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_AttemptGroupAssoc_EX2';
insert into  GT_RAD_AttemptGroupAssoc_EX2
with nonnewgroups as (select customerkey, collectionkey from GT_RAD_newattempts_ex1 where new_group_flag <> 1),
currentnewgroup as (select n.customerkey, n.collectionkey,
    ...
    OVER (partition by a.customerkey order by a.collectionkey RANGE BETWEEN 0 preceding and 1 following) PrevGoodKey
        from GT_RAD_newattempts_ex1 a LEFT OUTER JOIN nonnewgroups n ON a.customerkey = n.customerkey
     ...
     --  select * from currentnewgroup where customerkey = 47123559 order by collectionkey
       --select * from GT_RAD_newattempts_ex1  collectionkey = 203180107 --customerkey is not null  
...
select distinct GT_RAD_newattempts_ex1.customerkey,
        GT_RAD_newattempts_ex1.collectionkey,
        CASE when New_Group_flag = 1 then GT_RAD_newattempts_ex1.collectionkey -- if it's a new group, it's part of this
         ...
         --    CASE when New_Group_flag = 1 then 'New' --GT_RAD_newattempts_ex1.collectionkey -- if it's a new group, it's part of this
 ...
 from GT_RAD_newattempts_ex1
 LEFT OUTER JOIN firstnewgroup on GT_RAD_newattempts_ex1.collectionkey = firstnewgroup.collectionkey
 LEFT OUTER JOIN previousgroup ON GT_RAD_newattempts_ex1.customerkey = previousgroup.customerkey;
 ...
 --where collectionkey in (select collectionkey from thing group by collectionkey having count(*) > 1)
  --select count(*) from GT_RAD_newattempts_ex1
...
--select max(collectiongroupkey) from GT_RAD_Active_Collection_Groups 203280036
...
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_newattempts_stats_ex3';
insert into  GT_RAD_newattempts_stats_ex3  
select  ex2.collectiongroupkey, 
        count(ex2.collectionkey) NewAttempts, 
        max(ex2.collectionkey) MostRecentAttempt
from GT_RAD_AttemptGroupAssoc_EX2 ex2
...
--
-- Table:GT_RAD_active_groups_ex1
--
-- HEY -- Want to change the sydate here eventually
-- It will close out items incorrectly if you are running old data
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_active_groups_ex1';
insert into GT_RAD_active_groups_ex1
...
--
-- Table: GT_RAD_New_Collection_Groups
-- select * from GT_RAD_new_collection_groups order by customerkey, collectiongroupkey
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_New_Collection_Groups'; --to be _EX4
insert into GT_RAD_New_Collection_Groups
    select customerkey, 
...
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_New_Collection_Groups_EX1';
insert into   GT_RAD_New_Coll_Groups_EX1
    select customerkey, 
    CollectionGroup,
    ...
    max(CollectionGroupStartDate) OVER 
     (partition by customerkey order by CollectionGroupKey RANGE BETWEEN 0 preceding and 1 following)  
        NextGroupStartDate
    from GT_RAD_New_Collection_Groups;
 --commit;
 
 
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_New_Coll_Groups_EX3';
insert into GT_RAD_New_Coll_Groups_EX3
--as
--with RAD_Collection_Groups as
--(
--select * from GT_RAD_Active_Collection_Groups 
--union all -- shouldn't be any duplicates
--select * from GT_RAD_Final_Collection_Groups 
--)
...
JOIN GT_RAD_New_Coll_Groups_EX1 ON GT_RAD_New_Coll_Groups_EX1.customerkey = RAD_Collection_Groups.customerkey
...
-- (where applicable)
-- Table: GT_RAD_Active_Collection_Groups
...
insert into  ODS_central.RAD_Collection_Groups 
    select     
    ex1.customerkey, 
    ex1.collectiongroup +nvl(prevcollectiongroup,0), 
    ex1.collectiongroupstartdate, 
    ...
    CASE when collected_flag = 1 then 1 else 0 END final_status_flag,
    sysdate -- process date time
    from ODS_central.GT_RAD_New_Coll_Groups_EX1 ex1
        JOIN ODS_central.GT_RAD_newattempts_ex1 on GT_RAD_newattempts_ex1.collectionkey=ex1.collectiongroupkey
        LEFT OUTER JOIN  ODS_central.GT_RAD_New_Coll_Groups_EX3 ex3 ON ex3.customerkey = ex1.customerkey;
--commit;
...
-- Table: GT_RAD_Active_Collection_Groups
-- 
--EXECUTE IMMEDIATE 'truncate table ODS_CENTRAL.GT_RAD_New_Group_Stats' ;
 insert into ODS_central.GT_RAD_New_Group_Stats
   ...
   -- co.exclusion_reason,
    ...
    from ODS_central.GT_RAD_newattempts_ex1 co
    JOIN ODS_central.GT_RAD_AttemptGroupAssoc_EX2 ex2 ON EX2.COLLECTIONKEY= CO.COLLECTIONKEY 
    JOIN ODS_central.GT_RAD_newattempts_stats_ex3  ex3 ON EX2.COLLECTIONGROUPKEY= ex3.collectiongroupkey
    JOIN  ODS_central.RAD_Collection_Groups cg    ON cg.collectiongroupkey = ex2.collectiongroupkey
...
-- Get Finalized actives - 3 ways
 ...
 UPDATE ODS_central.RAD_Collection_Groups
...
-- Option 1a: If there is a new group...well, it's probably already in there but update it. 
 ...
 (
  with newgroups as
(select GT_RAD_newattempts_ex1.customerkey, GT_RAD_newattempts_ex1.collectionkey, GT_RAD_newattempts_ex1.new_group_flag, collected_flag, exclusion_reason, collectiongroupkey  collectiongroupkey
     from ODS_central.GT_RAD_newattempts_ex1
    LEFT OUTER JOIN ODS_central.GT_RAD_AttemptGroupAssoc_EX2 ex2 ON ex2.collectionkey = GT_RAD_newattempts_ex1.collectionkey
   where exclusion_reason is null
    )
 ...
 /* index(RAD_Collection_Groups)*/ --newgroups
    ...
    --JOIN GT_RAD_New_Coll_Groups_EX3 ex3 ON ex3. 
    --JOIN GT_RAD_newattempts_ex1 ON 
...
-- Table: GT_RAD_Active_Collection_Groups
-- GT_RAD_Collection_Group_Disp
-- Figure updated and new dispositions
-- Now item 2 in list of five post-grouping steps
--EXECUTE IMMEDIATE 'truncate table ods_central.GT_RAD_Collection_Group_Disp_CF1';
 INSERT into ODS_central.GT_RAD_coll_group_disp_CF1
    ...
    from ODS_central.GT_RAD_new_group_stats
        Where new_group_flag =1 and collected_flag = 1;
--commit;
--select * from GT_RAD_new_group_stats
--select * from GT_RAD_coll_group_disp_CF1

insert into ODS_central.GT_RAD_coll_group_disp_CF1
--create table   GT_RAD_coll_group_disp_CF1 as select * from GT_RAD_Collection_Group_Disp
(select distinct  GT_RAD_New_Group_Stats.customerkey, GT_RAD_New_Group_Stats.collectiongroup, GT_RAD_New_Group_Stats.collectiongroupstartdate, GT_RAD_New_Group_Stats.collectiongroupkey, GT_RAD_New_Group_Stats.nextgroupkey, GT_RAD_New_Group_Stats.nextgroupstartdate, GT_RAD_New_Group_Stats.newattempts, GT_RAD_New_Group_Stats.mostrecentattempt,
--GT_RAD_New_Group_Stats. 
--GT_RAD_New_Group_Stats.*,    
...
CASE when DISP.RESPONSE='A'  then 'Paid'        
    when DISP.RESPONSE='D' and retrycount < 4 and collectiongroup = max(collectiongroup) OVER (partition by GT_RAD_New_Group_Stats.customerkey) then 'Declining'        
    else 'Failed' END 
        Group_Disposition,        
CASE when DISP.RESPONSE='A' and disp.retrycount =0 and disp.processorgroup in ('B','Z') then 'Paid on first attempt'        
    ...
    when DISP.RESPONSE='D' and retrycount < 4 and collectiongroup = max(collectiongroup) OVER (partition by GT_RAD_New_Group_Stats.customerkey) then 'Declining through try '||to_char(retrycount+1) || ' (Attempt in group  TBD)'      
    else 'Failed' END 
        Group_Disposition_Detail,
        0, -- Final_Status_flag, will handle later
        sysdate -- Process_Datetime        
from   ODS_central.GT_RAD_New_Group_Stats   
JOIN ODS_central.GT_RAD_dispattempts_ex2 disp 
    ON GT_RAD_New_Group_Stats.customerkey = disp.customerkey 
    --select * from GT_RAD_New_Group_Stats
...
--JOIN GT_RAD_newattempts_ex1 on GT_RAD_newattempts_ex1.collectionkey =     disp.collectionkey
...
UPDATE GT_RAD_Collection_Group_Disp
SET GT_RAD_COLLECTION_GROUP_DISP.nextgroupkey = GT_RAD_coll_group_disp_CF1.NextGroupKey 
--select * from GT_RAD_coll_group_disp_CF1,GT_RAD_Collection_Group_Disp
WHERE exists
(select 1 from GT_RAD_coll_group_disp_CF1.collectiongroupkey = GT_RAD_Collection_Group_Disp.COLLECTIONGROUPKEY
and   GT_RAD_Collection_Group_Disp.nextgroupkey < GT_RAD_coll_group_disp_CF1.NextGroupKey
*/
-- There are 6 possible updates,
-- 4 of them are 2 groups of 2, so 4 MERGE in total

MERGE INTO ODS_central.RAD_Collection_Group_Disp d
    USING (select * from ODS_central.GT_RAD_coll_group_disp_CF1) cf1
...
insert into ODS_central.RAD_Collection_Group_Disp -- just use this for now, eventually will flip it
select cf1.* from ODS_central.GT_RAD_coll_group_disp_CF1 cf1
JOIN ODS_central.GT_RAD_AttemptGroupAssoc_EX2 ON GT_RAD_AttemptGroupAssoc_EX2.collectiongroupkey = CF1.collectiongroupkey
JOIN ODS_central.GT_RAD_newattempts_ex1  ON GT_RAD_newattempts_ex1.collectionkey = GT_RAD_AttemptGroupAssoc_EX2.collectionkey
...
--   
 ...
 where final_status_flag = 0
 and exists
    (select 1 from GT_RAD_active_groups_ex1 ex1--,RAD_Collection_Groups
...
*/
    --commit;
--select * from GT_RAD_New_Coll_Groups_EX3
 
 --create table GT_RAD_Final as 

END IF; -- Test
insert into ODS_central.RAD_Final
    select
   GT_RAD_newattempts_ex1.customerkey,
   GT_RAD_newattempts_ex1.collectionkey,
    MarketingRegion,
    BillingCountry,
    currencycode,
   ...
   daystocollect,
   declinesuntilsuccessful,
dense_rank() over (partition by RAD_Collection_Group_Disp.collectiongroupkey order by GT_RAD_newattempts_ex1.collectionkey) AttemptNumberinGroup, 
...
from ODS_central.RAD_Collection_Group_Disp  -- RIGHT OUTER JOIN custsubset ON    --   and
JOIN ODS_central.GT_RAD_AttemptGroupAssoc_EX2 ON GT_RAD_AttemptGroupAssoc_EX2.collectiongroupkey = RAD_Collection_Group_Disp.collectiongroupkey
JOIN ODS_central.GT_RAD_newattempts_ex1  ON GT_RAD_newattempts_ex1.collectionkey = GT_RAD_AttemptGroupAssoc_EX2.collectionkey
--JOIN 
--AND
--(
--Collection record is within the current group and not equal to the next group
 --(GT_RAD_newattempts_ex1.collectionkey >= RAD_Collection_Group_Disp.collectiongroupkey and GT_RAD_newattempts_ex1.collectionkey < RAD_Collection_Group_Disp.nextgroupkey )  
...
--( GT_RAD_newattempts_ex1.collectiongroupkey = RAD_Collection_Group_Disp .nextgroupkey and  GT_RAD_newattempts_ex1.collectionkey >=RAD_Collection_Group_Disp.nextgroupkey)
--
--) --select * from GT_RAD_AttemptGroupAssoc_EX2

;
--commit;

/*
create table GT_RAD_Final_Collection_Groups as
select * from ra_nextcgroups  
where ra_nextcgroups.customerkey < 1000
create table GT_RAD_Active_Collection_Groups as
...
-- Table: GT_RAD_Active_Dispositions
-- 2 monts, 6:52 minutes. That should be fine.



/*
insert into  GT_RAD_Final_Collection_Groups 
    select 
    ex1.customerkey, 
    ex1.collectiongroup+nvl(prevcollectiongroup,0), 
    ex1.collectiongroupstartdate, 
    ...
    ex1.nextgroupkey, 
    ex1.nextgroupstartdate
    from GT_RAD_New_Coll_Groups_EX1 ex1
    JOIN GT_RAD_newattempts_ex1 on GT_RAD_newattempts_ex1.collectionkey=ex1.collectiongroupkey
    LEFT OUTER JOIN  GT_RAD_New_Coll_Groups_EX3 ex3 ON ex3.customerkey = ex1.customerkey
    and collected_flag = 1;
--commit;
insert into  GT_RAD_Active_Collection_Groups 
    select     
    ex1.customerkey, 
    ex1.collectiongroup +nvl(prevcollectiongroup,0), 
    ex1.collectiongroupstartdate, 
    ex1.collectiongroupkey, 
    ex1.nextgroupkey, 
    ex1.nextgroupstartdate from GT_RAD_New_Coll_Groups_EX1 ex1
    JOIN GT_RAD_newattempts_ex1 on GT_RAD_newattempts_ex1.collectionkey=ex1.collectiongroupkey
    LEFT OUTER JOIN  GT_RAD_New_Coll_Groups_EX3 ex3 ON ex3.customerkey = ex1.customerkey
    and collected_flag <> 1;
--commit;
